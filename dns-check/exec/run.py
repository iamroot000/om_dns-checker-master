...
class greatChecker(object):
    ...
    def get_dom(self):
        mydb = self.conn()
        cur = mydb.cursor()
        cur.execute("SELECT domain FROM SSLDOMAINS_ssldomain2 where skip='no'")
    ...
    def lezdodiz(self):
        exluded_dom = self.get_cdndom() + self.get_exdom()
        domains = set(self.get_dom()) - set(exluded_dom)
        self.loggingFile(log_debug="{}".format("#####" * 20))
            ...
            try:
                ...
                for i2 in range(0, 3):
                    try:
                        ...
                        idc = str(idc)
                        self.loggingFile(log_debug="{}".format("#####" * 20))
                        self.loggingFile(log_debug="TRY {}: IDC API check OK".format(i2), log_info=str("IDC Output is {}".format(idc)))
                        break
                    except Exception as e:
                        print('Timedout check 8002')
                        idc = 'error'
                        self.loggingFile(log_debug="{}".format("#####" * 20))
                        self.loggingFile(log_debug="TRY {}: IDC API check ERROR".format(i2), log_error=str(e))
